...
class FocusedOrangeGaur(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.stopOrderPrice = None

    def OnData(self, data: Slice):
        # wati 30 days after last exit
        ...
        if self.stopMarketTicket is not None and self.Portfolio.Invested:
            if price > self.highestPrice:
                ...
                self.stopOrderPrice = 0.95 * price
                updateFields = UpdateOrderFields()
                updateFields.StopPrice = self.stopOrderPrice
    ...
    def OnOrderEvent(self, orderEvent):
        ...
        if self.entryTicket is not None and self.entryTicket.OrderId == orderEvent.OrderId:
            self.stopOrderPrice = 0.95 * self.entryTicket.AverageFillPrice
